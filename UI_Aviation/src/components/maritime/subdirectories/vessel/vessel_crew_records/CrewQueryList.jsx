...
function CrewQueryList() {
  ...
  const { identification_number, shipimo } = useSelector(
  ...
  const fetchPerson = async (page) => {
    try {
      ...
      const promises = Object.keys(response).map((item) =>
        fetchPersonByIdentification(response[item].identificationnumber).then(
          (personData) => ({
            ...response[item],
      ...
      if (error.response && error.response.status === 403) {
        ...
        console.error(
          "An error occurred during personlistget:",
  ...
  useEffect(() => {
    const filteredRelations = (row) => {
      ...
      return (
        ...
        (!imoFilter || row.imo.toString().includes(imoFilter)) &&
        (!relationTypeFilter ||
  ...
  const columns = useMemo(
    () => [
      ...
      {
        name: "Gender",
        selector: (row) => row?.person_data.gender,
        sortable: true,
        width: "100px",
      },
      {
