...
import { closeMscDelete } from "../../../store/maritime/msc/MscActionSlice";
...
function MscDelete({ deleteRow }) {
  const dispatch = useDispatch();
  const { rowData, isDeleteVisible } = useSelector((state) => state.msc_action);
  const toastRef = useContext(ToastContext);
  const toastBottomCenter = useRef(null);
  const {access_token} = useSelector((state)=> state.auth)

  useEffect(() => {}, [rowData, isDeleteVisible]);
  ...
  const deleteHandle = async (event) => {
    ...
    try {
      ...
      const response = await MscService.deleteMsc(rowData.id, access_token);

      console.log("Msc deleted successfully:", response);
      toastRef.current.show({
        severity: "success",
        summary: "Operation Successful",
        detail: "Vessel has been deleted successfully.",
        life: 3000,
      });
      ...
      toastRef.current.show({
        ...
        detail: "Vessel has not been deleted.",
        life: 3000,
      });
    }

    dispatch(closeMscDelete());
  };
  ...
  return (
    ...
    >
      ...
      <Dialog
        ...
        onHide={() => dispatch(closeMscDelete())}
        footer={
          <div id="modal_footer" className="flex justify-end gap-3 mt-3">
            ...
            <Button
              ...
              ring-2 ring-red-900"
              label="No"
              onClick={() => dispatch(closeMscDelete())}
            />
          </div>
        }
        closeIcon
